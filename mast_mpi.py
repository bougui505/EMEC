...
class Chains(object):
    ...
    def fix_chains(self):
        ...
        for chainid in self.chainids:
            ...
            new_fragments.extend(numpy.split(self.maligns[chainid].adjmat.max_score().coords,
                                             cutting_spot))
...
class CAtoAll(object):
    ...
    def refine(self, chainid):
        ...
        aa = all_atoms.AllAtoms('chain_%s.pdb'%chainid, self.config.nc,
...
if __name__ == '__main__':
    ...
    #        score = chains.maligns[chainid].adjmat.max_score().score
    #        n_align = chains.maligns[chainid].aln.n_align
    #        npos = chains.maligns[chainid].aln.npos
    #        coverage = float(n_align)/npos
    if RANK == 0:
        # Write the chains object to disk
        with open('chains.pkl', 'wb') as outputpkl:
            cPickle.dump(chains, outputpkl, 1)
