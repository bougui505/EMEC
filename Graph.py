...
def flatten_coo_matrix(coomat):
    """
    Flatten the given sparse matrix in coo format
    """
    nx, ny = coomat.shape
    data, row = [], []
    for pointer, (i, j) in enumerate(zip(coomat.row, coomat.col)):
        row.append(numpy.ravel_multi_index((i, j), (nx, ny)))
        data.append(coomat.data[pointer])
    col = [
        0,
    ] * len(row)
    return scipy.sparse.coo_matrix((data, (row, col)), shape=(nx * ny, 1))


def argsort_coo_matrix(coomat):
    """
    Argsort the flatten coomat
    """
...
class Graph:
    ...
    def get_graph(self, adjacency_matrix=None):
        ...
        for i in range(adjacency_matrix.shape[0]):
            for j in adjacency_matrix[i].indices:
                self.add_edge(graph, i, j, adjacency_matrix[i, j])
        return graph

    def make_sets(self):
        self.sets = []
        for v in range(self.adjacency_matrix.shape[0]):
            self.sets.append({v})
    ...
    def get_minimum_spanning_tree(self):
        ...
        """
        self.make_sets()
        sorter = argsort_coo_matrix(self.adjacency_matrix)
        nx, ny = self.adjacency_matrix.shape
        row, col, data = [], [], []
        adjmat = self.adjacency_matrix.tocsr()
        progress = progress_reporting.Progress(len(sorter), label="MS-tree")
        for i in sorter:
            ...
            if index_set1 != index_set2:
                ...
                row.append(u)
                col.append(v)
                data.append(adjmat[u, v])
                row.append(v)
                col.append(u)
                data.append(adjmat[u, v])
            progress.count()
        self.minimum_spanning_tree = scipy.sparse.coo_matrix((data, (row, col)),\
    ...
    def umatrix(self):
        ...
        umat = numpy.ma.filled(
            numpy.mean(numpy.ma.masked_array(self.adjacency_matrix, self.adjacency_matrix == numpy.inf),
            ...
            for i in mstree[current.index].keys():
                ...
                if numpy.isinf(n.distance):
                    if pdist is None:
                        ...
                        n.distance = current.distance + pdist[current.index, n.index]
                    n.parent = current
                    Q.append(n)
        # Sort nodes by ascending distances:
        nodes = numpy.asarray(list(nodes.values()))
    ...
    def dijkstra(self, starting_cell=None, break_at_local_min=False, get_predecessors=False, max_distance=numpy.inf):
        """
